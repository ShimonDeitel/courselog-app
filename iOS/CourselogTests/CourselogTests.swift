import XCTest
@testable import Courselog

@MainActor
final class CourselogTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(CourseEntry(title: "New One"))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testDeleteEntryDecreasesCount() {
        store.add(CourseEntry(title: "ToDelete"))
        let before = store.entries.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        store.isPro = false
        store.entries = (0..<Store.freeLimit).map { CourseEntry(title: "Item \($0)") }
    ...
    func testUpdateEntryChangesTitle() {
        store.add(CourseEntry(title: "Original"))
    ...
    func testDeleteAtOffsetsRemovesCorrectEntry() {
        store.entries = []
        store.add(CourseEntry(title: "A"))
        store.add(CourseEntry(title: "B"))
